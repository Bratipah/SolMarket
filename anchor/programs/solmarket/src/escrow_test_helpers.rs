...
pub fn setup_escrow_test() -> EscrowTestEnvironment {
    ...
    // Create token mints
    let token_mint_a = create_token_mint(&mut litesvm, &mint_authority, 9, mint_authority).unwrap();
    let token_mint_b = create_token_mint(&mut litesvm, &mint_authority, 9, mint_authority, ).unwrap();
