...
import { SOLMARKET_PROGRAM_ADDRESS } from '../programs';
...
export type IncrementInstruction<
  TProgram extends string = typeof SOLMARKET_PROGRAM_ADDRESS,
  TAccountSolmarket extends string | AccountMeta<string> = string,
  TRemainingAccounts extends readonly AccountMeta<string>[] = [],
> = Instruction<TProgram> &
  InstructionWithData<ReadonlyUint8Array> &
  InstructionWithAccounts<
    [
      TAccountSolmarket extends string
        ? WritableAccount<TAccountSolmarket>
        : TAccountSolmarket,
      ...TRemainingAccounts,
    ]
  >;
...
export type IncrementInput<TAccountSolmarket extends string = string> = {
  solmarket: Address<TAccountSolmarket>;
};

export function getIncrementInstruction<
  TAccountSolmarket extends string,
  TProgramAddress extends Address = typeof SOLMARKET_PROGRAM_ADDRESS,
>(
  input: IncrementInput<TAccountSolmarket>,
  config?: { programAddress?: TProgramAddress }
): IncrementInstruction<TProgramAddress, TAccountSolmarket> {
  // Program address.
  const programAddress = config?.programAddress ?? SOLMARKET_PROGRAM_ADDRESS;

  // Original accounts.
  const originalAccounts = {
    solmarket: { value: input.solmarket ?? null, isWritable: true },
  };
  ...
  const instruction = {
    accounts: [getAccountMeta(accounts.solmarket)],
    programAddress,
    data: getIncrementInstructionDataEncoder().encode({}),
  } as IncrementInstruction<TProgramAddress, TAccountSolmarket>;
